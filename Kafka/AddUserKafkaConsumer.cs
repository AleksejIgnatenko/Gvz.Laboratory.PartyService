...
{
    public class AddUserKafkaConsumer : IHostedService
    {
        ...
        public AddUserKafkaConsumer(ConsumerConfig config, IUserRepository userRepository)
        {
            _config = config;
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _consumer.Subscribe("add-user-topic");
            ...
            try
            {
                while (true)
                {
                    try
                    {
                        ...
                        var addUserDto = JsonSerializer.Deserialize<UserDto>(cr.Message.Value)
                            ?? throw new InvalidOperationException("Deserialization failed: UserDto is null.");

                        var addManufacturerId = await _userRepository.CreateUserAsync(addUserDto);
